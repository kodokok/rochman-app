...
@section('page-action')
    <input id="save" type="submit" value="Save" class="btn btn-success float-right"
        style="width: 120px;">
    <a id="cancel" href="{{ route('auditplan.index') }}" class="btn btn-secondary float-right mr-2"
        style="width: 120px;">Cancel</a>
    @if ($model->exists)
        @hasanyrole('admin|auditor_lead|auditor')
            @if (!empty($model->ubahJadwalAudit->exists))
                <button id="ubah-jadwal" class="btn btn-warning float-right mr-2"  style="width: 150px;">Update Jadwal</button>
            @endif
        @else
            @if ($is_real_kadept)
                <button id="ubah-jadwal" class="btn btn-warning float-right mr-2"  style="width: 120px;">Ubah Jadwal</button>
            @endif
        @endhasanyrole
    @endif
@endsection
